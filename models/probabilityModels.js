function fatorial(n) {
  return n <= 1 ? 1 : Array.from({ length: n }, (_, i) => i + 1).reduce((a, b) => a * b);
}

function calcularBinomial(n, k, p, q) {
  const combinacao = fatorial(n) / (fatorial(k) * fatorial(n - k));
  return combinacao * Math.pow(p, k) * Math.pow(q, n - k);
}

function calcularResultados(n, p, q, kValores) {
  let somaProbabilidades = 0;

  kValores.forEach(k => {
    if (k >= 0 && k <= n) {
      somaProbabilidades += calcularBinomial(n, k, p, q);
    }
  });
  ...
  return {
    probabilidade: somaProbabilidades * 100,
    media,
    dp,
    cv
  };
...
// Tabela da distribuição normal padrão — importante incluir ela completa aqui
const Tabela = [
...
function buscarNaTabela(z) {
  ...
  const linha = parseInt(match[1] + match[2]);
  const coluna = parseInt(match[3]) + 1;
...
function calcularProbabilidadeNormal(tipo, media, dp, intervalo) {
  const valores = intervalo.split(';').map(Number);

  if (tipo === 'Menor que') {
    const z = calcularZ(valores[0], media, dp);
    const p = buscarNaTabela(z);
    if (p === null) return null;
    return z < 0 ? (0.5 - p) * 100 : (0.5 + p) * 100;
  }
  ...
  if (tipo === 'Entre dois números') {
    const z1 = calcularZ(valores[0], media, dp);
    const z2 = calcularZ(valores[1], media, dp);
...
function calcularUniforme(a, b, intervaloArray, medida) {
  let probabilidade = 0;
  ...
  const media = (a + b) / 2;
  const desvioPadrao = Math.sqrt(((b - a) ** 2) / 12);
